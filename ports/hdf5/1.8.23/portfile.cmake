...
vcpkg_from_github(
  ...
  "${hdf5_ref}"
  SHA512
  3bbf39f94f7370989e27d17f083c11b9f58fefb75d79aa88bc91d51df022efb00c63579286fdaf2e025e2e880beb33ababd706d8303f84e19b1e0274256d0a03
  HEAD_REF
  develop
  PATCHES
  hdf5_config.patch
...
if("parallel" IN_LIST FEATURES AND "cpp" IN_LIST FEATURES)
  message(
    WARNING
      "Feature 'Parallel' and 'cpp' are mutually exclusive, enabling option ALLOW_UNSUPPORTED automatically to enable them both."
  )
  set(ALLOW_UNSUPPORTED ON)
endif()

if("threadsafe" IN_LIST FEATURES
   AND ("parallel" IN_LIST FEATURES
        OR "fortran" IN_LIST FEATURES
        OR "cpp" IN_LIST FEATURES))
  message(
    WARNING
      "Feture 'threadsafe' and other features are mutually exclusive, enable feature ALLOW_UNSUPPORTED automatically to enable them both."
  )
  set(ALLOW_UNSUPPORTED ON)
endif()
...
if("fortran" IN_LIST FEATURE)
  message(
    WARNING
      "Feature 'fortran' is not yet official supported within VCPKG. Build will most likly fail if ninja 1.10 and a Fortran compiler are not available."
  )
endif()
...
if(NOT VCPKG_LIBRARY_LINKAGE STREQUAL "static")
  list(APPEND FEATURE_OPTIONS -DBUILD_STATIC_LIBS=OFF -DONLY_SHARED_LIBS=ON)
endif()
...
string(
  REPLACE [[${HDF5_PACKAGE_NAME}_TOOLS_DIR "${PACKAGE_PREFIX_DIR}/bin"]]
...
if(HDF5_TOOLS)
  ...
  foreach(tool h5cc h5pcc h5hlcc)
    if(EXISTS "${CURRENT_PACKAGES_DIR}/tools/${PORT}/${tool}")
      vcpkg_replace_string("${CURRENT_PACKAGES_DIR}/tools/${PORT}/${tool}"
                           "${CURRENT_INSTALLED_DIR}" "$(dirname \"$0\")/../..")
    endif()
  endforeach()
  if(EXISTS "${CURRENT_PACKAGES_DIR}/bin/h5fuse.sh")
    file(RENAME "${CURRENT_PACKAGES_DIR}/bin/h5fuse.sh"
         "${CURRENT_PACKAGES_DIR}/tools/${PORT}/h5fuse.sh")
...
if("parallel" IN_LIST FEATURES)
  file(INSTALL "${CMAKE_CURRENT_LIST_DIR}/vcpkg-port-config.cmake"
       DESTINATION "${CURRENT_PACKAGES_DIR}/share/${PORT}")
endif()

if(VCPKG_LIBRARY_LINKAGE STREQUAL "dynamic")
  vcpkg_replace_string(
    "${CURRENT_PACKAGES_DIR}/include/H5public.h"
    "#define H5public_H"
    "#define H5public_H\n#ifndef H5_BUILT_AS_DYNAMIC_LIB\n#define H5_BUILT_AS_DYNAMIC_LIB\n#endif\n"
  )
endif()

file(RENAME "${CURRENT_PACKAGES_DIR}/share/${PORT}/data/COPYING"
